...
function out_metod()
{
    $tmp1[1] = get_global_var('metod');
    $tmp2[1] = get_global_var('metod1');
    $tmp3[1] = get_global_var('num_news');
    $m_metod[1] = $tmp1[1];
    $m_metod[2] = $tmp2[1];
    $m_metod[3] = $tmp3[1];
    unset($tmp1);
...
{
    $z = "select * from t_news where isshow='Y' and isactive='Y' and url_svyaz='' order by data_publ desc";
    $r = execute_sql($z);
    return $r;
}
...
{
    $z = "select * from t_news where isshow='Y' and isactive='Y' and url_svyaz='$url_svyaz' order by data_publ desc";
    $r = execute_sql($z);
    return $r;
}
...
//       $num_news=1;
    ...
    if ($m_url_svyaz[1] != '')
        echo "<tr><td><img id='break' name='break' border='0' src='img/break_small_news.gif'></td></tr>";
        ...
        if ($corner == 'top')     //Строим таблицу где картинка сверху от новости
        {
            ...
            if ($m_url_svyaz[$i] == '') {
                ...
                if ($m_url[$i] == '') {
                    ...
                    echo "<tr class='$clas'><td class='$clas'>$m_b_descr[$i]</td></tr><tr><td><img id='break' name='break' border='0' src='img/break_small_news.gif'></td></tr>";
                    ...
                    if ($m_url_word[$i] != '')
                        ...
                        echo "<tr class='$clas'><td class='$clas'><a  class='$clas' href='$m_url[$i]' target='$target'>$m_b_descr[$i]</a></td></tr><tr><td><img id='break' name='break' border='0' src='img/break_small_news.gif'></td></tr>";
